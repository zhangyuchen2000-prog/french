...
function todaysWords(date = new Date()) {
  ...
  const words = saved.map(fr => WORDS.find(word => word.fr === fr)).filter(Boolean);
  return words.length === 5 ? words : defaultWords(date);
}
function randomWordsExcluding(currentWords) {
  const current = new Set(currentWords.map(word => word.fr));
  const preferred = WORDS.filter(word => !current.has(word.fr));
  const pool = preferred.length >= 5 ? preferred : WORDS.slice();
...
function showNewWords() {
  ...
  state.batches[dateKey()] = next.map(word => word.fr);
  save();
  render();
}
